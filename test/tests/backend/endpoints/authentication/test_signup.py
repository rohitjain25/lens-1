...
from tests.backend.constants import Schema_Files, Status_Code
...
def test_auth_signup(framework):
    ...
    log.info("Deleting user if already created")
    ...
    log.info("Making a POST request to /endpoints/auth/signup")
    response = requests.post(url, json.dumps(payload))
    actual_response = response.json()
    assert 201 == response.status_code, log.info(
...
def test_auth_signup_twice(framework):
    ...
    response = requests.post(url, json.dumps(payload))
    actual_response = response.json()
    assert Status_Code.CREATED == response.status_code, print(
    ...
    response = requests.post(url, json.dumps(payload))
    actual_response = response.json()
    assert Status_Code.CONFLICT == response.status_code, print(
...
def test_auth_signup_verbs(framework):
    ...
    assert Status_Code.METHOD_NOT_ALLOWED == response.status_code, print(
...
def test_auth_signup_invalid_password(framework):
    ...
    print("Call /endpoints/auth/signup with invalid credentials")
    payload = {
    ...
    assert Status_Code.BAD_REQUEST == response.status_code, print(
...
def test_auth_signup_invalid_first_name(framework):
    ...
    print("Call /endpoints/auth/signup with invalid first_name")
    payload = {
        "email": "test123@lens.com",
        "password": "test",
        "last_name": "String",
